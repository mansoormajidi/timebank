...
type Step = 'list' | 'detail' | 'processing' | 'receipt';
...
export function BillFlow({ onClose, onComplete }: { onClose: () => void; onComplete: (event: ActivityEvent) => void }) {
  ...
  function back() {
    ...
    if (step === 'detail') { setError(''); setStep('list'); }
    else if (step === 'receipt') onClose();
    else onClose();
  }
  const title = step === 'list' ? 'پرداخت قبض‌ها' : step === 'detail' ? 'جزئیات قبض' : step === 'processing' ? 'پرداخت قبض' : 'رسید پرداخت قبض';
