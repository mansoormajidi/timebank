...
function NotificationsPanel() {
  ...
  const items = tab === 'private' ? [
    ...
    { icon: 'receipt', title: 'امامزاده صالح عزیز تجریش', description: 'مشارکت شما با موفقیت ثبت شد.', time: 'امروز، ۱۰:۳۵' },
    { icon: 'blocked', title: 'مسدودی مبلغ', description: 'حساب نبض زمان و خرج', time: 'دیروز، ۱۸:۲۰' },
  ] as const : [
...
export function HomeDetails({ id, hidden, onOpen, onClose, activities, onComplete, cardBlocked, theme, onThemeChange, avatar, onAvatarChange, onLogout }: { id: string; hidden: boolean; onOpen: (id: string) => void; onClose: () => void; activities:Activity[]; onComplete:(a:ActivityEvent)=>void; cardBlocked:boolean; theme:'light'|'dark'; onThemeChange:(theme:'light'|'dark')=>void; avatar:string; onAvatarChange:(avatar:string)=>void; onLogout:()=>void }) {
  ...
  if (id === 'banking-locked') return <div className="banking-locked-sheet"><IdentityAnimation /><h3>برای این کار، احراز هویت لازم است</h3><p>بعد از احراز هویت، حساب بانکی شما افتتاح می‌شود و می‌توانید کارت دلخواهتان را درخواست دهید.</p><ActionButton shape="pill" className="primary-button" onClick={() => onOpen('identity')}>شروع احراز هویت</ActionButton></div>;
  ...
  if (id === 'giving' || id === 'endowment') return <><img className="giving-detail-art" src={`/assets/${id === 'giving' ? 'imgRectangle2' : 'imgRectangle1'}.png`} alt="" /><p className="detail-amount">{id === 'giving' ? '۳ بار حس خوب' : hidden ? '••٬•••٬•••' : '۳۵٬۶۵۸٬۰۰۰ ریال'}</p><p className="sheet-description">{id === 'giving' ? 'اینجا ردپای همراهی‌ها و هدیه‌هایی که ساختی رو می‌بینی؛ قدم‌هایی که تأثیرشون از بین نمیره!' : 'کار ماندگار نوعی سرمایه‌گذاری عام‌المنفعه است که اصل مشارکت با رعایت نرخ تنزیل حفظ و فواید حاصل از آن به نیازمندان تعلق خواهد گرفت.'}</p><ActionButton shape="pill" className="primary-button" onClick={() => onOpen(id + '-start')}>{id === 'giving' ? 'انجام کار خوب' : 'انجام کار ماندگار'}</ActionButton><p className="sheet-note">اطلاعات نمونه هستند و پرداختی انجام نشده است.</p></>;
  if (id === 'profile') {
  ...
  if (id === 'profile-about') return <div className="about-timebank"><span className="about-logo"><Icon name="logo" size={38}/></span><h3>بانکداری و خدمات اوقاف در یک مسیر</h3><p>تایم‌بانک اینترنت‌بانک سازمان اوقاف است؛ فضایی یکپارچه برای انجام امور بانکی روزمره و دسترسی به خدمات مرتبط با اوقاف. از جابه‌جایی وجه و مدیریت حساب تا پرداخت اجاره‌نامه، قبض و مشارکت در کارهای نیک، همه در تجربه‌ای ساده و امن کنار هم قرار گرفته‌اند.</p><p>این نسخه یک پروتوتایپ تعاملی است و به سامانهٔ بانکی واقعی متصل نیست.</p></div>;
  if (id === 'profile-version') return <VersionPanel/>;
  if (id === 'profile-devices') return <DeviceList/>;
  const descriptions: Record<string, string> = {
    accounts: 'مدیریت حساب‌ها و افتتاح حساب در مرحلهٔ بعد اضافه می‌شود.', transfer: 'جابه‌جایی کارت‌به‌کارت، پایا و ساتنا در مرحلهٔ بعد پیاده‌سازی می‌شود.',
    funding: 'سرمایه‌گذاری در پروژه‌های تأمین مالی با تضمین بانک مرکزی؛ جزئیات پروژه‌ها به‌زودی ارائه می‌شود.',
    'gold-fund': 'سرمایه‌گذاری آسان و مرحله‌ای در صندوق‌های مبتنی بر طلا، به‌زودی در میزفرمان فعال می‌شود.',
    token: 'مدیریت توکن‌ها و دارایی‌های دیجیتال تایم‌بانک، به‌زودی از همین مسیر در دسترس خواهد بود.',
    'time-fund': 'صندوق زمان برای پس‌انداز هدفمند و برنامه‌های آینده طراحی شده و به‌زودی فعال می‌شود.',
