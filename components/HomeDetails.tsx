...
export function HomeDetails({ id, hidden, onOpen, onClose, activities, onComplete, cardBlocked }: { id: string; hidden: boolean; onOpen: (id: string) => void; onClose: () => void; activities:Activity[]; onComplete:(a:ActivityEvent)=>void; cardBlocked:boolean }) {
  ...
  if (id === 'giving' || id === 'endowment') return <><img className="giving-detail-art" src={`/assets/${id === 'giving' ? 'imgRectangle2' : 'imgRectangle1'}.png`} alt="" /><p className="detail-amount">{id === 'giving' ? '۳ کمک و هدیه' : hidden ? '••٬•••٬•••' : '۳۵٬۶۵۸٬۰۰۰ ریال'}</p><p className="sheet-description">{id === 'giving' ? 'کمک‌های کوچک، اثرهای ماندگار. این بخش خلاصه‌ای از هدایا و مشارکت‌های شما را نشان می‌دهد.' : 'خلاصهٔ مشارکت در وقف پول؛ جزئیات طرح‌های وقف در مرحلهٔ بعد اضافه می‌شود.'}</p><button className="primary-button" onClick={() => onOpen(id + '-start')}>{id === 'giving' ? 'انجام کار خوب' : 'انجام کار ماندگار'}</button><p className="sheet-note">اطلاعات نمونه هستند و پرداختی انجام نشده است.</p></>;
  if (id === 'profile') return <><div className="profile-detail"><img src="/assets/avatar.png" alt="تصویر کاربر در طرح Figma" /><strong>کاربر نمایشی</strong><span>به تایم‌بانک خوش آمدید</span></div><p className="sheet-description">ورود و ثبت‌نام را با اطلاعات آزمایشی تجربه کنید.</p><button className="primary-button" onClick={() => onOpen('login')}>ورود به حساب</button><button className="secondary-button" onClick={() => onOpen('signup')}>ثبت‌نام</button><p className="sheet-note">این نسخه به هیچ حساب بانکی متصل نیست.</p></>;
