...
import { AuthFlow, type AuthenticatedUser } from '../components/AuthFlow';
...
export default function Home() {
  ...
  const {state:cardIssuance,requestCard,activateCard}=useCardIssuance(currentUser?.nationalId || '');
  const [bankingReady, setBankingReady] = useState(false);
  const hasBankingAccess = bankingReady || cardIssuance.status !== 'none';
  const cardBlocked=activities.find(a=>a.title==='مسدودی گرین‌کارت'||a.title==='رفع مسدودی گرین‌کارت')?.title==='مسدودی گرین‌کارت';
  const [boot,setBoot]=useState(true);
  const [screen, setScreen] = useState<'home' | 'login' | 'signup' | 'transfer' | 'statement' | 'topup' | 'bill' | 'card-issuance' | 'identity' | 'services' | 'assistant' | 'support'>('login');
  ...
  const completeBankingIdentity = useCallback(() => {
    if (!currentUser) return;
    setBankingReady(true);
    try { localStorage.setItem(`timebank-banking-ready-v2:${currentUser.nationalId}`, 'true'); } catch {}
  }, [currentUser]);
  const completeAuthentication = useCallback((user: AuthenticatedUser) => {
    setCurrentUser(user);
    try { setBankingReady(localStorage.getItem(`timebank-banking-ready-v2:${user.nationalId}`) === 'true'); }
    catch { setBankingReady(false); }
    setBoot(false);
    setScreen('home');
    setHomeEntry(entry => entry + 1);
    setScrolled(false);
  }, []);
  function open(id: string) {
    if (!hasBankingAccess && ['transfer', 'statement', 'topup', 'numbers', 'report', 'card', 'card-issuance'].includes(id)) { setDetail('banking-locked'); return; }
    if (id === 'login' || id === 'signup' || id === 'transfer' || id === 'statement' || id === 'topup' || id === 'bill' || id === 'card-issuance' || id === 'identity' || id === 'services' || id === 'assistant' || id === 'support') { setDetail(null); setScreen(id); return; }
    setDetail(id); if (id === 'notifications') setNotificationsRead(true);
  }
  function navigate(id: string) {
    setActive(id);
    if (id === 'home') { setDetail(null); setScreen('home'); setHomeEntry(e => e + 1); requestAnimationFrame(() => document.getElementById('home-scroll')?.scrollTo({ top: 0, behavior: reduced ? 'auto' : 'smooth' })); }
    else if (id === 'services' || id === 'assistant' || id === 'support') { setDetail(null); setScreen(id); }
  }
  return <MotionConfig reducedMotion="user"><div className="preview-stage">
    ...
    <main className="app-shell" dir="rtl">
      {screen === 'transfer' ? <TransferFlow onComplete={record} onClose={() => { setScreen('home'); setActive('home'); setScrolled(false); setHomeEntry(e => e + 1); }} /> : screen === 'statement' ? <StatementFlow activities={activities} hidden={hidden} onClose={() => { setScreen('home'); setActive('home'); setHomeEntry(e => e + 1); }} /> : screen === 'topup' ? <TopUpFlow onComplete={record} onClose={() => { setScreen('home'); setActive('home'); setHomeEntry(e => e + 1); }} /> : screen === 'bill' ? <BillFlow onComplete={record} onClose={() => { setScreen('home'); setActive('home'); setHomeEntry(e => e + 1); }} /> : screen === 'card-issuance' ? <CardIssuanceFlow issuance={cardIssuance} onRequest={requestCard} onActivate={activateCard} onComplete={record} onClose={() => { setScreen('home'); setActive('home'); setHomeEntry(e => e + 1); }} /> : screen === 'identity' ? <IdentityVerificationFlow issuance={cardIssuance} onRequest={requestCard} onActivate={activateCard} onComplete={record} onVerified={completeBankingIdentity} onClose={() => { setScreen('home'); setActive('home'); setHomeEntry(e => e + 1); }} /> : screen === 'services' ? <ServicesHub onClose={() => navigate('home')} onOpenBill={() => open('bill')} /> : screen === 'assistant' || screen === 'support' ? <NavigationPage kind={screen} onClose={() => navigate('home')} /> : screen !== 'home' ? <AuthFlow key={screen} initialMode={screen} playSplash={boot} onClose={completeAuthentication} /> : <div className="home-screen" key={homeEntry}>
      ...
      <div onScroll={e => setScrolled(e.currentTarget.scrollTop > 12)} className="app-scroll home-enter" id="home-scroll" inert={!!detail} aria-hidden={detail ? true : undefined}>
        <section className="hero">
          ...
          {hasBankingAccess ? <div className="balance-block"><h1>{account.name}</h1><div className="balance-number"><strong>{hidden ? '•••٬•••٬•••' : account.balance}</strong><Icon name="rial" size={24} /></div><button className="report-button" onClick={() => open('report')}>کنترل مسیر<Icon name="chevron" size={12} /></button></div> : <div className="balance-block banking-onboarding"><h1>برای استفاده از خدمات بانکی، احراز هویت کنید</h1><p>هنوز حساب بانکی فعالی ندارید.</p><button className="identity-cta" onClick={() => open('identity')}>شروع احراز هویت<Icon name="chevron" size={13}/></button></div>}
        ...
        <div className="home-content flex flex-col gap-4">
          ...
          <section className="glass-card command-desk"><h2 className="section-title">میزفرمان</h2><div className="daily-services">{services.map(s => <button key={s.id} onClick={() => open(s.id)}><span className="service-icon"><Icon name={s.icon} /></span><span>{s.title}</span></button>)}</div></section>
          {hasBankingAccess && cardIssuance.status !== 'none' && <HomeCardCarousel issuance={cardIssuance} hidden={hidden} blocked={cardBlocked} onOpenIssuance={() => open('card-issuance')} onOpenCard={() => open('card')} />}
          ...
          <div className="giving-grid"><button className="glass-card giving-card" onClick={() => open('giving')}><span className="section-title">کار خوب<Icon name="chevron" size={12} /></span><span className="giving-content"><img src="/assets/imgRectangle2.png" alt="" /><span><strong>۳</strong><small>بار حس خوب</small></span></span></button><button className="glass-card giving-card" onClick={() => open('endowment')}><span className="section-title">کار ماندگار<Icon name="chevron" size={12} /></span><span className="giving-content"><img src="/assets/imgRectangle1.png" alt="" /><span><strong className="endowment-value">{hidden ? '••٬•••٬•••' : '۳۵٬۶۵۸٬۰۰۰'}</strong><small>سرمایه‌گذاری عام‌المنفعه · ریال</small></span></span></button></div>
          <section className="glass-card connected-services"><h2 className="section-title">خدمات ویژه</h2><button className="connected-row" onClick={() => open('bill')}><span className="service-icon"><Icon name="receipt" /></span><span><strong>پرداخت قبض</strong><small>پرداخت قبوض آب، برق، گاز، تلفن، اوقاف و…</small></span><Icon name="chevron" size={16} /></button><button className="connected-row" onClick={() => open('funding')}><span className="service-icon"><Icon name="funding" /></span><span><strong>تأمین مالی</strong><small>سرمایه‌گذاری در پروژه‌های تأمین مالی با تضمین بانک مرکزی</small></span><Icon name="chevron" size={16} /></button></section>
    ...
    {screen !== 'login' && screen !== 'signup' && <div inert={!!detail} aria-hidden={detail?true:undefined}><BottomNavigation active={active} onNavigate={navigate} /></div>}
